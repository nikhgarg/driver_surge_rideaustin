...
def pipeline_data_preprocessing(settings, runlabel):
    folder = settings["folder"]

    df = create_clean_rides_file.clean_and_merge_datafiles(
        folder, runlabel, override_start_with_dispatch=settings.get("override_start_with_dispatch", False)
    )
...
def pipeline(settings):
    ...
    earnings_period_start_time = settings.get("earnings_period_start_time", "start_hour")
    ...
    if skip_mimicfare_in_plot_stuff:
        ...
        payment_function_names_plotting = payment_function_names
    ...
    if "pipeline_data_preprocessing" in functions_to_run:
        ...
        pipeline_data_preprocessing(settings, settings["preprocessed_data_filelabel"])

    if "plot_drivershift_earnings" in functions_to_run:
        df = load_data(
            folder=folder, filename="{}{}.csv".format(settings["preprocessed_data_filelabel"], settings["fileending_with_start_df"])
        )
        ...
        for shift_column in settings["driver_shift_columns"]:
            ...
            plot_weighted_histogram_session_earnings(
                df_session,
                payment_function_names_plotting,
                plt_label=outputrunlabel,
                save_fig=True,
                # colors=settings.get("plot_colors", None),
            )
    ...
    if "plot_drivershift_earnings_variancebyaddmult" in functions_to_run:
        ...
        for shift_column in settings["driver_shift_columns"]:
            ...
            plot_weighted_variance_session_earnings_by_addmult_interpolation(
                df_session, settings["payment_function_add"], settings["payment_function_mult"], plt_label=outputrunlabel, save_fig=True
            )
    ...
    if "get_driver_trips_in_period_for_matches" in functions_to_run:
        ...
        for number_hours_next in numbers_hours_next:
            for match_function_name in match_functions_names_local:
                ...
                par = partial(
                    add_trips_in_period_fixed_time_difference_from_row,
                    number_hours_next=number_hours_next,
                    df_grouped=df_grouped,
                    df=df,
                    match_column=match_function_name,
                    start_time_col="start_hour",
                )

                df.loc[
                    :, "{}_tripsinperiod_{}".format(match_function_name.replace("_matched_index", ""), number_hours_next)
                ] = parallelize_on_rows(df, par)
    ...
    if "get_earnings_in_period" in functions_to_run:
        ...
        for number_hours_next in numbers_hours_next:
            for match_function_name in match_functions_names_local:
                ...
                df.loc[:, trips_list_column] = df[trips_list_column].apply(
                ...
                aggstrings = [
                    "{}_{}_earnings{}".format(match_function_name.replace("_matched_index", ""), func_name, number_hours_next)
                    for func_name in payment_function_names
                ]

                par = partial(
                    get_period_earnings_withmatchlists,
                    df=df,
                    trips_list_column=trips_list_column,
                    payment_function_names=payment_function_names,
                    number_hours_next=number_hours_next,
                    start_time_col=earnings_period_start_time,
                )
    ...
    if "plot_tripindifference_histogram" in functions_to_run:
        ...
        tripindifylim = settings.get("ylim", (-10, 50))
        ...
        for number_hours_next in numbers_hours_next:
            for match_function in match_functions:
                ...
                plot_trip_indifference_variance_by_addmult_interpolation(
                    df,
                    settings["payment_function_add"],
                    settings["payment_function_mult"],
                    number_hours_next,
                    match_function,
                    limit_to_matched_drivers_who_had_trip=True,
                    func_filesave_names=func_filesave_names,
    ...
    if "supplementary_facts" in functions_to_run:
        print("starting supplementary facts")
        df = pd.read_csv("{}{}_with_basicearnings_inperiod.csv".format(folder, runlabel))
        supplementary_pipeline(
            outputrunlabel,
            df,
            match_functions,
            payment_functions_names=payment_function_names,
            payment_functions_prettynames=func_pretty_names,
        )


if __name__ == "__main__":
    for settings in [settings_server_2months, settings_plotting_2months]:
        pipeline(settings)
