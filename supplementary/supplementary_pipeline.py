...
def scatter_fares(runlabel, df, farex = 'mimic_fare', farey = 'pure_addsurge_bysurgefactor_fare', namex = 'Reverse engineered fare', namey = 'Additive Surge'):
...
def surge_basic_facts(runlabel, df):
    ...
    sns.lineplot(x = 'surge_factor', y = 'minutes', data = df)
    plt.xlabel('Surge Factor', fontsize = 25)
...
def surge_evolution(runlabel, df):
    ...
    df.loc[:, 'Surge Bin'], retbins = pd.cut(df['surge_factor'],bins = [1, 1.5, 2, 2.5, 3, 5], retbins = True, precision = 1, include_lowest  = True, right = False)
    ...
    #Surge lag in period
    df.loc[:, 'rounded_time_to_10_minutes'] = df.eval('(dispatch_hour*60)//6*10') #rounded every 10 minutes
    ...
    dflocrounded.loc[:, 'Surge Bin'], retbins = pd.cut(dflocrounded['surge_factor'],bins = [1, 1.5, 2, 2.5, 3, 5], retbins = True, precision = 1, include_lowest  = True, right = False)
    ...
    labels = ('Current surge in [1, 1.5)', 'Current surge in [1.5, 2)', 'Current surge in [2, 2.5)', 'Current surge in [2.5, 3)', 'Current surge in [3, 5)')
    ...
    df.loc[:, 'Hour in week'] = (dtindex.apply(lambda x: x.hour))*(dtindex.apply(lambda x: x.weekday())+1)
...
def surge_close_to_capitol_markovian(runlabel, df):
    ...
    print(df3days.head())
    df3days.loc[:,'miles_from_capitol'] = df3days.apply(lambda x: haversine(lat_capitol, long_capitol, x['start_location_lat'], x['start_location_long']), axis = 1)
    df3days_closetocap = df3days.query('miles_from_capitol < 5')
    df3days_closetocap.loc[:, 'rounded_time_to_10_minutes'] = df3days_closetocap.eval('((dispatch_hour*60)//6)/10 - @minn - @offset') #rounded every 10 minutes
...
def supplementary_pipeline(outputrunlabel, df, matching_functions):
